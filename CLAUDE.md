...
> **Status (refreshed Revision 1.10, July 2026).** **v0.1 is feature-complete and v0.2 is underway.** The full spine — parse → pre-flight → recovery → export → report → validation — plus the Information Discovery Engine and the `xtalate` CLI shipped for the four v0.1 formats (XYZ, extXYZ, POSCAR, CONTCAR). v0.2 progress: **M7** completed the full Part 4 §3.3 recovery scenario catalog (Slices 1–2, Revisions 1.8–1.9), and **M8** added the POSCAR/CONTCAR velocity block (Cartesian + Direct) and the velocity/mass recovery family — `missing_velocities` (`zero_init`/`maxwell_boltzmann`/`upload_reference`/`omit`) and `missing_masses` (`standard_masses`/`manual_input`), opt-in and chain-aware (Revision 1.10). The map below is current against Revisions 1.3–1.10 and `docs/DECISIONS.md` **D1–D47**. Mission, principles (P1–P6), the Mission Scope & Non-Goals above, glossary, architecture, the absence convention, tech stack (a *destination*, not the v0.1 dependency set), and API conventions remain binding. For build-time rationale read the MASTER_SPEC Preface revision log and the **D-log through D47**; per-version execution lives in `docs/IMPLEMENTATION_PLAN_v0.1.md`–`_v1.0.md`. (Revision 1.10 / D43–D47: M8 velocity/mass recovery — notably `maxwell_boltzmann` emits the raw sample with **no** COM-drift removal, the transparent-converter precedent; see `CHANGELOG.md` Unreleased.)
