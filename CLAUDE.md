...
> **Status (refreshed Revision 1.11, July 2026).** **v0.1 is feature-complete and v0.2 is underway.** The full spine — parse → pre-flight → recovery → export → report → validation — plus the Information Discovery Engine and the `xtalate` CLI shipped for the four v0.1 formats (XYZ, extXYZ, POSCAR, CONTCAR). v0.2 progress: **M7** completed the full Part 4 §3.3 recovery scenario catalog (Slices 1–2, Revisions 1.8–1.9); **M8** added the POSCAR/CONTCAR velocity block (Cartesian + Direct) and the velocity/mass recovery family — `missing_velocities` (`zero_init`/`maxwell_boltzmann`/`upload_reference`/`omit`) and `missing_masses` (`standard_masses`/`manual_input`), opt-in and chain-aware (Revision 1.10); and **M9** added the cross-format round-trip matrix — two-hop (`A→B→Canonical′`) and three-hop (`A→B→A`) suites whose comparable subspace is computed from the Capability Matrix, never hand-listed, plus custom tolerance-table files (`--tolerance-profile FILE`, YAML/JSON), which adds **PyYAML** as the first new runtime dependency since ASE (Revision 1.11); and **M10** added the report-completeness property test (`tests/property/`) — the test-time generalization of the M4 runtime completeness assertion, with both properties (completeness invariant + absence conformance) re-derived independently in test code and driven by a **stage-1** golden-mutation generator (the whole optional-field lattice) **and a stage-2 `hypothesis` generator** over randomized objects with shrinking (`hypothesis` added as a test-only dev dependency). Stage 2 immediately caught three real `frame_selection`-interaction silent-loss/crash bugs — two `constraint_representation` completeness bugs (D50) and a `mixed`-cell `missing_lattice` recovery-detection gap now fixed by resolving `missing_lattice` lazily against the retained frame (fabricate / refuse / no-op) with the generator bound removed (Revision 1.12 / D50–D51). The map below is current against Revisions 1.3–1.12 and `docs/DECISIONS.md` **D1–D51**. Mission, principles (P1–P6), the Mission Scope & Non-Goals above, glossary, architecture, the absence convention, tech stack (a *destination*, not the v0.1 dependency set), and API conventions remain binding. For build-time rationale read the MASTER_SPEC Preface revision log and the **D-log through D51**; per-version execution lives in `docs/IMPLEMENTATION_PLAN_v0.1.md`–`_v1.0.md`. (Revision 1.12 / D50–D51: M10 report-completeness property test — stage-1 golden mutations + stage-2 hypothesis, both properties re-derived independently of the runtime guard; three frame-reduction bugs found and all fixed with regression fixtures; see `CHANGELOG.md` Unreleased.)
