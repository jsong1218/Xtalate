"""The reviewable fuzz seed battery — pure data, shared by the pytest corpus and the generator.

These deterministic malformed-byte seeds are consumed by two callers with different dependency
footprints:

* :mod:`tests.fuzz.test_parser_fuzz` — the pytest corpus that asserts the graceful-failure
  contract (needs ``pytest``);
* :mod:`tests.fuzz.make_seed_corpus` — the ClusterFuzzLite seed-corpus generator, run at
  image-build time inside a fuzzing image that installs **only** the pure library (no ``pytest``,
  no dev extras — see ``.clusterfuzzlite/build.sh``).

Keeping the battery here, importing nothing beyond the standard library, is what lets the generator
read the seeds without dragging the test framework into the fuzzing image. It is deliberately data
only: the ``ParseError``-xor-``ParseResult`` assertion and the parametrized tests live next door in
:mod:`tests.fuzz.test_parser_fuzz`.
"""
