...
Self-hosting is the primary supported deployment: a hardened [`docker-compose.prod.yml`](docker-compose.prod.yml) points at external Postgres and S3-compatible storage. See the **[self-hosting guide](docs/self-hosting.md)**. There is also a **hosted demo** on Hugging Face Spaces — ephemeral, anonymous, with a 25 MB cap — whose deploy recipe lives in [`docs/huggingface-space.md`](docs/huggingface-space.md).
