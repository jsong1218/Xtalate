...
from xtalate.sdk import AnalysisPlugin, ExporterPlugin, ParserPlugin
...
ANALYSIS_ENTRY_POINT_GROUP = "xtalate.analysis"

#: The explicit first-party analysis-plugin list (MASTER_SPEC Part 3 §7.1's P6 rule: discovery is
#: additive, so the built-in list stays explicit and a first-party plugin never needs the
#: entry-point machinery to exist). **Empty in v1.8 M68** — the milestone ships the seam, not the
#: science: the first-party reference plugin (``xtalate-analysis-composition``, M69) is its own
#: installable distribution and arrives through the entry-point pass below exactly like a
#: third-party one, which is what makes it a test of the seam rather than a privileged built-in.
BUILTIN_ANALYSIS_PLUGINS: tuple[AnalysisPlugin, ...] = ()
...
def default_registry() -> Registry:
    """A fresh :class:`Registry` with all built-in parsers, exporters, and analysis plugins
    registered, followed by any third-party plugins advertised via entry points (Part 3 §7.1).
    ...
    *additive* — the built-in lists stay explicit (**P6**), and an installation with no plugins
    behaves exactly as before the mechanism existed (the analysis list is empty in v1.8 M68, so
    ``analysis_plugins()`` is ``[]`` until a distribution advertises one).
    """
    registry = Registry()
    ...
    for analysis_plugin in BUILTIN_ANALYSIS_PLUGINS:
        registry.register_analysis_plugin(analysis_plugin)
    _register_from_entry_points(registry)
    return registry


def _register_from_entry_points(registry: Registry) -> None:
    """Discover and register every parser/exporter/analysis plugin advertised under the Xtalate
    entry-point groups. Each plugin passes through the *same* ``register_*`` path as a built-in,
    so it gets duplicate detection and capability-declaration validation for free (Part 3 §7.1).
    ...
    The third group (``xtalate.analysis``, v1.8 M68) is the same pass, one more loop — the
    duplicate guard, the ``PluginLoadError`` wrapping, and the fail-loudly-on-a-broken-plugin
    behavior are inherited from the parser/exporter loops (D60–D62), not rebuilt for analysis.
    """
    ...
    for ep in entry_points(group=ANALYSIS_ENTRY_POINT_GROUP):
        plugin = _load_plugin(ep)
        if not isinstance(plugin, AnalysisPlugin):
            raise PluginLoadError(
                f"{_describe(ep)} produced {type(plugin).__name__}, not an AnalysisPlugin"
            )
        _register(registry.register_analysis_plugin, plugin, ep)


_P = TypeVar("_P", ParserPlugin, ExporterPlugin, AnalysisPlugin)


def _register(register: Callable[[_P], None], plugin: _P, ep: EntryPoint) -> None:
    """Register one discovered plugin, attributing a duplicate-id/-name rejection to its
