...
_EXPANSION_NOTE = (
    ...
    "positions within {threshold} Å. Coordinates generated by an operation "
...
_CONSUMED_SITE_TAGS = frozenset(
    {
        ...
        "_atom_site_cartn_y",
        "_atom_site_cartn_z",
        # Mapped to the first-class atoms.occupancies field (M35), so it is excluded from the
        # generic carry to keep one source column from arriving under two names.
        OCCUPANCY_TAG,
    }
)
...
def build(
    ...
    if occupancy is not None:
        # Parse the declared column once (a bare '?'/'.' is already None from the document stage,
        # the schema's per-site "unknown"; a stated value becomes a finite float, a parenthesized
        # standard uncertainty read as its value like every other CIF number), then replicate onto
        # the expanded atoms through the same site_of mapping every per-site fact uses.
        ...
        for raw in occupancy:
            ...
            if has_uncertainty(raw):
                uncertain.add(OCCUPANCY_TAG)
            declared.append(parse_number(raw, tag=OCCUPANCY_TAG, line=loop.line))
        occupancies = [declared[i] for i in site_of]
        # Occupancy is a first-class field now (M35), so its mere presence is no longer a schema
        # gap and CIF_OCCUPANCY_NOT_MODELLED is retired. What still earns a warning is *partial*
        # occupancy — a statement about this structure's chemistry (real disorder): the file's own
        # _chemical_formula_sum reflects the fractional sites, but the Canonical Object carries a
        # whole atom at each, so its composition is the fully-occupied one. An unknown site
        # ('?'/'.') counts as partial: silence is not a claim of fullness (P4).
        partial = sum(1 for value in occupancies if not is_full_occupancy(value))
        if partial:
            partial_note = _PARTIAL_OCCUPANCY_NOTE.format(partial=partial, total=len(occupancies))
            issues.append(
                ParseIssue(
                    severity="warning",
