        ...
        * ``STRESS_CARRY_DROPPED`` — a frame carries **both** a populated ``electronic.stress``
        ...
        if carry_values is not None:
            for frame in canonical.frames:
                ...
                # length-9 array on .traj write; that value cannot be judged against the
                # written tensor — skip the drop-check rather than broadcast-crash (ASEDB-1,
                # review R4): the field is still written, the unrecognized carry is simply
                # not compared.
