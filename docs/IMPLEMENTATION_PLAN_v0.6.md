...
2. **Upload (`/convert`):** drop zone with the instance's limits shown inline *before* failure is possible (from `GET /v1/limits`: "Files up to 500 MB · auto-deleted after 7 days · delete anytime"); upload progress from progress events; failure states through the M27 envelope component. On `201`, route to `/files/[file_id]`, which submits `POST /v1/inspect` (idempotent per the v0.5 contract, so revisits are cheap).
