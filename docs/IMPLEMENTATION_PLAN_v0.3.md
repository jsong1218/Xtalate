...
1. **Parser/exporter, ASE-backed** (D7: ASE is the only scientific dependency), covering the format's breadth per Part 3 §3: positions/symbols/masses, cells, velocities, forces, energy, stress, charges, magnetic moments, constraints (ASE `FixAtoms` → `Constraint(kind="fixed_atoms_mask")` *[correction, post-v0.3 architectural review: the shipped kind is `"fixed_atoms"` (D58, CHANGELOG `[0.3.0]`, MASTER_SPEC Revision 1.16); `"fixed_atoms_mask"` is the extXYZ move-mask kind (Part 3 §3 n.6) — this plan conflated the two]*), per-frame `time`/`trajectory.timestep`, calculator name/parameters → `simulation.calculator` / `simulation.extra` (n.10).
