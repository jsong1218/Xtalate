...
> - **M14C — DONE** (branch `m14-ase-traj`): golden fixtures + identity round-trip + matrix
>   membership. Two governed golden cases under `tests/golden/ase_traj/` — `co-relax-3frame` (rich
>   all-fields anchor: cell/pbc, isotope masses, velocities, forces+energy, FixAtoms, per-frame
>   `step`) and `water-single-molecule` (laundering anchor: every ASE default → `None`, trajectory
>   `None`) — regenerated by committed `_generate.py` (ULM bytes are byte-deterministic), each with a
>   manifest (synthetic/Apache-2.0) + refreshed `ATTRIBUTIONS.md`. New `tests/golden/
>   test_ase_traj_golden.py` (parse/streamed/identity + two field-assertion tests); `ase_traj`
