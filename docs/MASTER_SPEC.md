...
7. **Revision 1.2 — pre-implementation architecture review.** A lead-architect review conducted before any code was written (recorded in full in `docs/ARCHITECTURE_REVIEW.md`, alongside `docs/IMPLEMENTATION_PLAN_v0.1.md`) found a further set of internal inconsistencies and omissions, corrected here in four tranches. **Source-of-truth correction:** the split-vs-merged framing from the original drafting stage — the claim that eleven standalone `00`–`10` files are "the editable source of truth" and this file "is regenerated from them" — is retired (Preface); `docs/MASTER_SPEC.md` is now edited directly as the single source of truth, and Part 1 §5's repository tree no longer lists the nonexistent standalone files. **Worked-example correction:** Part 2 §8.2's POSCAR title line and Part 5 §6's corresponding `metadata_preservation` check are moved to `user_metadata.custom_global["poscar:comment"]`, matching the carry-through routing rule exactly as Part 2 §6.1 already states it (both previously and inconsistently showed `simulation.extra["poscar:comment_line"]`, contradicting §6.1's own worked distinction between free text and structural parameters). **Schema pre-release versioning:** worked examples now carry `schema_version: "0.1.0"` rather than `"1.0.0"` (Part 2 §5) — the schema reaching `1.0.0` is itself a v1.0 deliverable (`10 §6` item 3), so no pre-1.0 object should claim it; the generic illustrative mentions of "e.g. `1.0.0`" as a semver *shape* (Part 2 §§3.2, 5) are left as shape examples, not version claims. **Discovery Report presence trichotomy:** `FieldPresenceEntry.status` (Part 3 §6.2) gains `"mixed"` and an optional `present_frames`, matching the trichotomy `field_presence()` already defines (Part 2 §3.11) — a per-frame field present in only some frames of a trajectory was previously unrepresentable in a Discovery Report. **Capability Matrix:** the `fields` key convention now states the `"<category>.*"` wildcard's meaning explicitly (Part 3 §4.1), and the Part 3 §3 format table gains its missing `electronic.total_spin` row. **Dependency graph:** Part 1 §5.1's acyclic rule now names `plugin-sdk` explicitly, states that `FormatCapabilities`/`FieldCapability`/`CapabilityLevel` are defined in `plugin-sdk` (so every parser/exporter can return them without depending on the matrix package) and merely assembled and queried by `capability-matrix`, and gives the Information Discovery Engine/Format Sniffer (`packages/discovery`) and the CLI (`packages/cli`) their previously unplaced homes in the repository tree — both fully specified elsewhere (Part 3 §6, Appendix A) but absent from Part 1 §5 until now. **Packaging:** a clarifying note (Part 1 §5.2) states that `packages/*` are subpackages of one publishable distribution, `xtalate`, not separate PyPI distributions — resolving an ambiguity between Part 1 §5.1's "standalone, importable" language and Part 9 §3's singular "the `xtalate` distribution." **Version-ladder terminology:** Part 10 §1 gains a note acknowledging that `docs/Incremental_Roadmap_v1.0.md` re-slices this Part's v0.1–v1.0 labels into v0.1–v0.7, and disambiguating what "MVP" means across the two documents. **Design-intent scoping:** Parts 6, 7, and 9's status headers now distinguish contracts and vocabulary (binding) from implementation detail specified many versions ahead of when it will be built (design intent, re-validated when that version's work begins) — reducing the drift surface Part 10 §6 item 7 treats as a release blocker. **Minor clarifications:** `Constraint.parameters` must be JSON-serializable (Part 2 §3.6); `TrajectoryMetadata`'s single-field shape is an intentional seam for future trajectory-level metadata, not an oversight (Part 2 §3.5); sniff confidence values are ordinal signals pinned by the golden corpus, not calibrated probabilities (Part 3 §6.1); entry-point-based plugin discovery (Part 3 §7.1) is a mechanism for when third-party plugins exist — an explicit first-party registration list is sufficient before then and does not require this section's machinery. **Flagged, deliberately not resolved:** the repository's `LICENSE` file (MIT) still conflicts with Part 10 §4.1's binding Apache-2.0 decision. This revision does not resolve it, because the choice belongs to the project owner, not to an editorial pass — it is recorded here as the one open item pending that decision, exactly the discipline this note itself asks future findings to follow. **Addendum (M0 scaffolding pass, following the review's acceptance):** (item 8) the previously-flagged license item is now **resolved — Apache-2.0**: the `LICENSE` file carries the Apache-2.0 text and a `NOTICE` file was added as the golden-corpus attribution home (Part 8 §3.2). (item 9) Part 1 §5's repository tree now shows the concrete realization of the single distribution: `src/xtalate/` with subpackages `schema`/`sdk`/`parsers`/`exporters`/`capabilities`/`discovery`/`conversion`/`recovery`/`validation`/`cli`, replacing the earlier `packages/<hyphenated-name>/` sketch whose directory names could not be Python import names; the descriptive component names (`canonical-schema`, `plugin-sdk`, `capability-matrix`) are retained in prose, with the tree stating the mapping. Parsers and exporters remain separate subpackages (not a combined `formats/`) to honor their distinct Part 1 §2 contracts. (item 10) the concrete build-time decisions the review left open — packaging, Python floor, build backend, tooling, CLI framework, per-format strategy, array-serialization/golden-equality — are recorded in `docs/DECISIONS.md` (D1–D10), each with a rejected alternative. (item 11) the Preface gains a "Working conventions for AI coding agents" note: commits must not attribute authorship to an AI, and the standing rules of `docs/DECISIONS.md` / `docs/IMPLEMENTATION_PLAN_v0.1.md` §4 govern agent-written code. **Addendum (M3 critical-path formats pass — XYZ + POSCAR/CONTCAR):** (item 12) the two hand-rolled parsers landed with stable, format-prefixed `ParseIssue` codes extending the single `XYZ_INCONSISTENT_ATOM_COUNT` named in Part 3 §5 rule 4 — XYZ adds `XYZ_EMPTY`/`XYZ_MALFORMED_HEADER`/`XYZ_INVALID_SYMBOL`/`XYZ_MALFORMED_COORDINATE`, and POSCAR/CONTCAR add `POSCAR_MISSING_SPECIES` (recoverable, `recovery_hint="supply_species"`, §3 n.1), `POSCAR_INCONSISTENT_ATOM_COUNT`, `POSCAR_MALFORMED`, `POSCAR_INVALID_SYMBOL`, and the warning `POSCAR_PREDICTOR_CORRECTOR_CARRIED` (§3 n.12); all obey the §5 error contract and are recorded with their rejected alternative in `docs/DECISIONS.md` D16. (item 13) three build-time decisions the spec left to implementation are recorded in `docs/DECISIONS.md` D13–D15: the POSCAR exporter writes Cartesian coordinates (D13, exact round-trips without matrix inversion), identity-round-trip equality is over scientific content excluding the legitimately-varying `provenance` (D14), and one POSCAR implementation is registered twice as `poscar`/`contcar` with sniff bias rather than duplicated (D15). Note the deliberate deferral: per `IMPLEMENTATION_PLAN_v0.1.md` M3's stated asymmetry, **M3c (extXYZ) is off the critical path** and is not yet implemented — the critical path (XYZ + POSCAR/CONTCAR) is complete and is all M4–M6 need to demonstrate the full pipeline including the flagship `missing_lattice` refusal; extXYZ proceeds in parallel or ships in v0.1.1, adding the ASE dependency (D4, D7) when it lands. **Addendum (M3c formats pass — extXYZ):** (item 14) extXYZ landed ASE-backed (D7), adding the sole scientific-I/O dependency `ase>=3.29,<4` to the runtime `dependencies` (`docs/DECISIONS.md` D17, which also records the `mypy` untyped-boundary handling). Its parser implements the **default-laundering** obligation of Part 3 §2 verbatim — ASE's invented zero cell → `cell=None`, synthesised zero momenta → `velocities=None`, computed-from-Z masses → `masses=None`, and an undeclared `pbc=` on a lattice-bearing frame takes `(T,T,T)` by extXYZ convention recorded in `parse_notes` (the §3 n.3 analogue) — with the highest-value laundering suite of Part 8 §1.1 as its tests. The field mapping is recorded in `docs/DECISIONS.md` D18 and carries **one deliberate deviation from the §4.2 read-side declaration**: `electronic.stress` (declared `partial` in §4.2) is *not* populated in v0.1 — ASE's stress sign convention cannot be reconciled with the canonical tension-positive convention (Part 2 §3.7.1) without a source-declared convention, so stress is carried verbatim to `user_metadata.custom_per_frame["extxyz:stress"]` instead (declared `none`, with a `lossy_notes` entry) rather than risk a silent sign flip; per-atom `charge`/`magmoms` columns *are* mapped (to `electronic.charges`/`magnetic_moments`) and `momenta` is unit-converted to canonical Å/fs. New format-prefixed `ParseIssue` codes (extending item 12's convention): `EXTXYZ_EMPTY`, `EXTXYZ_PARSE_ERROR`, `EXTXYZ_VARIABLE_ATOM_COUNT` (the canonical constant-N invariant of Part 2 §3.2 cannot represent a variable-atom-count trajectory), and the warnings `EXTXYZ_PER_FRAME_COLUMN_NOT_REPRESENTABLE` and `EXTXYZ_UNMAPPED_RESULT_CARRIED`. With extXYZ complete, all four v0.1 MVP formats (XYZ, extXYZ, POSCAR, CONTCAR) are implemented — M3 is done ahead of the plan's cut-line contingency. **Addendum (M4 Capability Matrix + Conversion Engine pass):** (item 15) the Conversion Engine (`xtalate.conversion`) landed with the pre-flight diff (Part 3 §4.3), the `write_plan` discipline (Part 4 §1), the `ConversionReport` schema verbatim (Part 4 §2), and the completeness invariant as an always-on runtime assertion (`CompletenessInvariantError`, review §4.5). Three build-time decisions are recorded in `docs/DECISIONS.md` D19–D21: `PARTIAL` capabilities are predicted Preserved with the condition surfaced as a `PARTIAL_CAPABILITY` warning rather than evaluated per-object (D19), the `write_plan` is materialized as a filtered Canonical Object (`canonical′`) handed to the unchanged `export(canonical, stream)` rather than passed as a side-channel argument — so `canonical′` is also the Validation Engine's expected object (D20) — and the derived `atoms.atomic_numbers` is excluded from the diff and invariant as a non-independent mirror of `atoms.symbols` (D21). New stable report warning codes `PARTIAL_CAPABILITY` and `FORMAT_LOSSY_NOTE` (both `source="capability"`), and the refusal code `RECOVERY_REQUIRED` for a conversion whose recovery scenarios M4 detects but cannot yet resolve (a `status="refused"` completed outcome, not an error, Part 4 §4). One capability-declaration correction: the POSCAR/CONTCAR **write-side** `user_metadata.custom_global` cell was `none` but the exporter does write the title (`poscar:comment`) and CONTCAR predictor block (`contcar:predictor_corrector`), so it is corrected to `partial` with a note naming exactly those keys — the write-side row is now faithful to Part 3 §4.2's "declared FULL/PARTIAL obliges writing it" contract. Recovery resolution, strict-mode loss gating, and the automatic final-step validation remain M5. The `examples/convert_extxyz_to_poscar.py` demo prints the full report for the flagship extXYZ→POSCAR conversion — the project's first end-to-end, demo-able artifact. **Addendum (M5 Recovery + Validation Engine pass):** (item 16) the Recovery Engine (`xtalate.recovery`) and Validation Engine (`xtalate.validation`) landed, completing the philosophy-critical milestone. Recovery implements the three-way hazard classification (bulk-reductive / selective-reductive / fabricative, Part 4 §3.1) with the two v0.1 scenarios `frame_selection` (selective-reductive) and `missing_lattice` (fabricative, `manual_input`/`bounding_box`), **preset-only** (`docs/DECISIONS.md` D22), computed option lists that honestly exclude unoffered choices (`non_periodic` is never offered for periodic-only POSCAR), the `frame_selection`→`missing_lattice` dependency order (a bounding box is computed on the selected frame), and structured all-or-nothing refusal (`RECOVERY_REQUIRED`) — every applied choice records an `Assumption` (and, for fabrication, a `SuppliedEntry`), never a silent default (**P4**). The Conversion Engine now routes unresolved scenarios through recovery, gates strict-mode reductive loss / parse warnings behind `acknowledge_loss` / `acknowledge_parse_warnings` (Part 4 §4), and invokes validation as its unconditional final step. Validation constructs the expected object as the already-materialized `canonical′` (D20), re-parses the output through the ordinary registry, and runs the full §2 check catalog (`atom_count`, `species_preservation`, `positions_rmsd`, `lattice_consistency`, `frame_count`, `numeric_field_fidelity`, `metadata_preservation`, `absence_conformance`, `report_consistency`) under named tolerance profiles (`default`/`strict`/`loose`) with the representational-bound formula (Part 5 §4). Five build-time decisions are recorded in `docs/DECISIONS.md` D22–D27: preset-only recovery (D22); the additive `ExporterPlugin.atom_permutation()` seam that supplies the species/position permutation map from the exporter's own element grouping (D23); the additive `FormatCapabilities.numeric_precision` map, with v0.1's full-precision POSCAR declaring none so bases govern (D24); the `atoms.masses` tolerance base absent from the §4.3 table (D25); validation reading the Conversion Report through a structural `ConversionReportView` Protocol so `validation` never imports `conversion`, preserving the acyclic layering (D26); and the v0.1 scoping of `absence_conformance` (preserved-and-removed paths are validated by `frame_count`, not asserted absent) and `report_consistency` (supplied-traceability re-verified; full preflight-vs-final diffing deferred) (D27). The demo now prints the Validation Report alongside the Conversion Report. With M5 done, the whole spine — parse → pre-flight → recovery → export → report → validation — is implemented and demonstrated end to end; only the CLI + packaging of M6 remain for v0.1. **Addendum (M6 CLI + packaging pass — v0.1 complete):** (item 17) the Information Discovery Engine (`xtalate.discovery`) and the `xtalate` CLI (`xtalate.cli`, Appendix A) landed, completing v0.1. The Discovery Engine drives the existing sniffer + parser registry (sniff → parse → introspect) and derives the `DiscoveryReport` (Part 3 §6.2) from `field_presence()`, so discovery is exactly as trustworthy as parsing; its `fields` inventory is complete over the fixed 16 canonical scientific leaf paths (§6.3), with metadata containers summarized under `structure`/`extras`. The CLI is a thin argparse presenter over the engines with the four commands `inspect`/`convert`/`validate`/`capabilities`, the preset-only `--recover` form of `recovery_choices`, and the CI-native exit-code contract of §A.2 (`0` ok · `2` refused · `3` validation failed · `4` parse error · `5` strict-mode warnings · `1` usage); `--json` emits the report schemas verbatim. `validate` ships both re-validation modes of Part 5 §4.5 — offline full re-parse (reconstructing the expected object from source + report via the new public `conversion.build_expected_object`) and stored-report re-thresholding (`validation.rethreshold`). Four build-time decisions are recorded in `docs/DECISIONS.md` D28–D30 (plus the offline-refusal note): the Discovery Report's fixed leaf-path list and the top-level `xtalate.registry.default_registry()` composition root (D28); offline full re-parse re-validation **refuses** rather than validate when the conversion fabricated fields, since a `bounding_box` lattice cannot be reconstructed from source + report in v0.1 — a documented gap whose closing seam is persisting recovery-supplied values in the report (D29); and the CLI presentation choices — remapping argparse's usage exit `2` to `1` so it cannot collide with the *refused* code, keeping an overridden format's real sniff score (with an `overridden` marker) rather than fabricating `1.0`, and `--json` emitting the pydantic reports verbatim (D30). Packaging: `__version__` is `0.1.0`, and `README.md`/`CHANGELOG.md`/`CITATION.cff` are written with the explicit "what v0.1 does and does not do" scope statement (the M3c cut was *not* taken — all four formats shipped). The git tag and PyPI/GitHub publish remain the maintainer's manual release step. **v0.1 is feature-complete: the entire spine and all four advisory subsystems are implemented, tested, and driveable from the command line.**
8. **Revision 1.3 — custom-array value-type broadening.** `user_metadata.custom_per_atom` and `custom_per_frame` were widened to accept lists of any JSON scalar (strings, booleans, integers, floats), not only numeric NumPy arrays, so that non-numeric per-atom/per-frame columns (e.g. string site labels) carry through verbatim rather than being forced into a numeric dtype or dropped. Recorded inline at Part 2 §3.10 and with its rejected alternative in `docs/DECISIONS.md` D12. (This entry is back-filled by Revision 1.6: the change predates Revisions 1.4/1.5 but had lived only as the §3.10 inline note, leaving the preface log non-monotonic — the log's completeness is itself part of the provenance claim this file makes.)
9. **Revision 1.4 — hosted-instance privacy and size posture.** A deliberate policy change (project owner's decision, recorded with its rejected alternatives in `docs/DECISIONS.md` D31), not a drift fix: the hosted instance becomes a deliberately small, aggressively private demonstration surface, and large files are explicitly the CLI's and self-hosting's job. Concretely: the default max upload size drops 500 MB → **100 MB**, with the `413 FILE_TOO_LARGE` path made a funnel to the free local path rather than a dead end (`details.self_hosting_url`; Part 6 §5, Part 7 §2.2); stored-byte retention drops 7 days → **30 minutes** across inputs, persisted Canonical Objects, and outputs, with outputs additionally deleted **immediately on first completed download** (Part 6 §5, Part 9 §5.2, Part 1 §4.3); the `awaiting_recovery` TTL drops 12 hours → **30 minutes**, with a paused job's `expires_at` capped by its input's so a paused job can never outlive the persisted Canonical Object it needs to resume (Part 6 §5); lifecycle enforcement inverts from bucket-rules-primary to **application-primary** (delete-on-download plus a monitored minute-cadence sweeper — a new fifth §6.1 alert) with a 1-day bucket rule retained as the platform backstop, because object-store lifecycle rules cannot express sub-day windows — the two-tier guarantee is stated honestly in Part 9 §5.2; and the advertised privacy copy is made normative there and deliberately does **not** claim "no data is retained" — reports outlive bytes by design, so the true claim is stated instead. Two `LimitsResponse` fields are renamed so their units stay honest rather than holding fractional values: `retention_days` → `retention_minutes` and `awaiting_recovery_ttl_hours` → `awaiting_recovery_ttl_minutes` (Part 6 §2.2, §5; propagated to `IMPLEMENTATION_PLAN_v0.5.md`) — a documentation-only rename, since Part 6 is design intent not yet built (its Revision 1.2 status header). Consumer copy is updated in Part 7 §2.2 and `IMPLEMENTATION_PLAN_v0.6.md`; Part 5 §4.5's re-parse re-validation window notes are rescaled accordingly. All values remain instance-configurable — self-hosters raise them by environment variable, and the library/CLI have no limits at all.
10. **Revision 1.5 — report retention (30 days, hosted) and zero-cost hosting topology.** Completes Revision 1.4's privacy posture by bounding the one artifact 1.4 left unbounded: on the hosted instance, conversion records and their reports are deleted **30 days** after completion — `report_retention_days` joins `LimitsResponse` and the Part 6 §5 limits table (`null` = indefinite, the natural self-host setting), enforcement joins the §5.2 retention sweeper at daily cadence, account deletion cascades immediately, and the normative privacy copy now states the 30-day report window explicitly rather than leaving "what persists" open-ended. The consequences are stated in place instead of implied: server-side re-thresholding and the history entry end with the report (Part 6 §2, Part 7 §2.6), re-thresholding's "always works" claim is scoped to the retention window (Part 5 §4.5, Part 1 §4.3), and the durable route is the user-downloaded report JSON plus the CLI's offline re-validation. Recorded with rejected alternatives in `docs/DECISIONS.md` D32. Separately, a candidate zero-cost hosting topology for the hosted instance — one always-free ARM VM running `docker-compose.prod.yml`, free-tier S3-compatible object storage, free static hosting for frontend and docs — is recorded as **design intent** in `docs/DECISIONS.md` D33 and pointed to from Part 9 §5.4: a platform pick made 12+ months before deployment is exactly the class of decision the Part 9 status header requires re-validating when the work begins.
11. **Revision 1.6 — post-v0.1 corpus consistency pass.** A full-corpus documentation review after v0.1 shipped (`docs/DOCS_CONSISTENCY_REVIEW_2026-07.md`, findings C1–C11) corrected drift between the spec's normative surfaces and shipped behavior. **extXYZ stress (C1):** the Part 3 §3 capability table cell and the §4.2 read-side declaration for `electronic.stress` are corrected from `partial`/◐ to `none`/○ (new footnote 14), matching the shipped parser and `docs/DECISIONS.md` D18 (itself recorded in Revision 1.2 item 14) — the Part 8 §1.1 table-vs-declaration sync test exists precisely to fail on that disagreement; stress is carried verbatim to `user_metadata.custom_per_frame["extxyz:stress"]`, and promotion to `electronic.stress` remains additive once a source-declared sign convention exists. **Version-ladder labels (C3):** Parts 6 and 7's status headers now use the roadmap ladder's labels — the Service API is roadmap v0.5 (was mislabeled with Part 10's old "v0.2") and the Web UI roadmap v0.6 (was "v0.5") — matching Part 10 §1's Revision 1.2 disambiguation note and `CLAUDE.md`'s usage. **Log hygiene (C4):** the missing Revision 1.3 preface entry (item 8 above) is back-filled from its Part 2 §3.10 inline note. **Logging convention, prospective (C10):** so that Revision 1.2 does not remain a ten-addendum container spanning the whole v0.1 build, from this revision onward each milestone's landing addendum takes its **own** revision number — one event per entry — keeping the log monotonic and one-event-per-line as it grows through v0.2–v1.0; history is not renumbered, since the record of how the log grew is itself provenance (Revision 1.1's own reasoning). The review's remaining findings land in their own files in the same change — the roadmap staleness banner (C2/C11), Revision 1.5 propagation into `IMPLEMENTATION_PLAN_v0.5.md`/`_v0.7.md` (C5), the D2 Python-floor review (C9), and the `CLAUDE.md` refresh (C6) — with a new standing rule in `docs/DECISIONS.md` that any revision touching Parts 6/7/9 names and edits its affected plan files in the same change. C7 (`Xtalate_Doc_Prompts.md`) and C8 (project instructions) concern artifacts outside this repository and are flagged, not edited here.

12. **Revision 1.7 — POSCAR scaling-factor routing correction (post-v0.1 code review).** A review that exercised the shipped v0.1 code against real inputs found the §8.2 worked example internally inconsistent with the §6.1 carry-through routing rule: the example filed the POSCAR scaling factor in `simulation.extra["poscar:scaling_factor"]`, but §6.1 admits `simulation.extra` for a structural parameter *only if it is not already reflected in the reconstructed lattice* — and the parser unconditionally folds the scale into the lattice vectors (§4), so that clause is never satisfied. Beyond the doc inconsistency this caused a real defect: `simulation.extra` is presence-bearing (§3.11) but no v0.1 exporter can write `simulation.*`, so the Validation Engine's `absence_conformance` check failed **every** POSCAR→POSCAR/CONTCAR conversion (the "removed" scale key necessarily reappears when the exporter writes its scale line). The §8.2 example now records the scaling factor as a `provenance` parse-note (provenance carries no ✓/✗ signal, §3.11), which is the honest classification of a folded-in scale; the golden fixtures and their tests are updated to match. Recorded with rejected alternatives in `docs/DECISIONS.md` D34. Seven further v0.1 code defects found in the same review are behavioral fixes with no normative-surface change (`CHANGELOG.md`, Unreleased).

13. **Revision 1.8 — recovery scenario catalog completion, Slice 1 (v0.2 M7).** The Recovery Engine, which v0.1 shipped resolving two scenarios preset-only (`missing_lattice`, `frame_selection` — the review §4.4 trim), grows to register and hazard-classify the **full Part 4 §3.3 catalog of eight scenarios**, so classification and the honest-option-list rule (Part 4 §3.3) are now mechanically complete for the four v0.1 formats. Per the delivery plan this lands in two slices; this is Slice 1 — the pre-flight-driven core. New this revision: `constraint_representation` resolves with `project`/`drop_all`; the remaining catalog scenarios (`missing_velocities`, `missing_masses`, `missing_energy`, `missing_species`, `truncate_corrupt_tail`) are registered and classified, with their resolvers deferred to M8 / Slice 2 as the plan prescribes, and honestly refusing until then. **One behavior change to a normative surface:** a source carrying a non-empty `dynamics.constraints` list converted to a target whose `dynamics.constraints` write-capability is **PARTIAL** (POSCAR, `selective_dynamics` only) no longer auto-Preserves — *which* constraints survive a partial translation changes the physics of a downstream relaxation, so it becomes a recorded `constraint_representation` choice (Part 4 §3.3), and such a conversion now refuses without an explicit preset. `PARTIAL`-vs-`NONE` is the discriminator: `NONE` stays ordinary bulk-reductive loss, `FULL` stays Preserved. Two supporting mechanisms are recorded with rejected alternatives in `docs/DECISIONS.md`: the machine-readable `allows_open_boundaries` write-capability flag that drives the ✳`non_periodic` option (D35), and the generalized dependency-ordered resolver table replacing the hard-coded two-scenario dispatch (D37); the constraint behavior change itself is D36.

14. **Revision 1.9 — recovery scenario catalog completion, Slice 2 (v0.2 M7).** Completes M7: the *invasive* pieces Revision 1.8 deferred now land, so every four-format-reachable choice in the Part 4 §3.3 catalog resolves. **Parse-time scenarios** — `missing_species` (VASP-4 POSCAR, `species_map`/`upload_reference`) and `truncate_corrupt_tail` (corrupt trajectory tail, `truncate`/`abort`) — fire before a Canonical Object exists and are resolved through a new optional `ParserPlugin.parse_recover` hook (additive to the frozen `parse` contract) and a `conversion.parse_with_recovery` orchestrator that threads the resulting Assumptions into the Conversion Report; a parse-time *fabricated* path (e.g. `atoms.symbols`) is recorded in `supplied` and treated as absent-at-source by the completeness invariant (`docs/DECISIONS.md` D38). **`upload_reference`** (lattice/species borrowed from a second structure) is now offered for `missing_lattice`/`missing_species`, with the second file parsed by the CLI into a `CanonicalObject` and structural atom-count/alignment guards in the resolver (D39). **`split_all`** (one output file per frame) closes the Slice-1 cut: it keeps every frame, adds `ConversionResult.outputs`, validates each file, and merges the per-file Validation Reports into one (D40) — so `frame_selection` now offers `split_all` wherever it triggers. No normative report/field shapes change; the `parse_recover` hook, the `ParseRecovery`/`parse_recovery` plumbing, and `ConversionResult.outputs` are additive. With Slice 2 the M7 catalog is mechanically complete for the four v0.1 formats.

15. **Revision 1.10 — POSCAR velocity block + velocity/mass recovery (v0.2 M8); mission-scope reinforcement.** The one deliberate v0.1 format deferral lands: POSCAR/CONTCAR velocities are read in both Cartesian and Direct conventions (Direct converted to Cartesian Å/fs via the lattice; absence preserved, never zero-filled), and the two velocity-family recovery scenarios resolve — `missing_velocities` (`zero_init`/`maxwell_boltzmann`/`upload_reference`/✳`omit`) and `missing_masses` (`standard_masses` from IUPAC atomic weights / `manual_input`), with the `maxwell_boltzmann → missing_masses` chain recording two ordered Assumptions. These are **opt-in** fabricative scenarios (the target does not *require* velocities/masses, so they fire only when the user supplies the choice), detected in `ConversionEngine.convert` rather than `build_preflight` to keep the pre-flight draft choice-independent (`docs/DECISIONS.md` D46); a fabricated field the target cannot store (chained masses on POSCAR) is audited in `supplied` but kept out of the write plan (D47). No normative report/field shapes change. **Mission reinforcement (no new normative surface, but stated so it binds):** this revision hardens Part 0 §4's "Not a molecular dynamics package" non-goal into an operative rule — Xtalate performs **no unrequested transformation, even a standard MD one, when it would obfuscate or hide information**. The worked precedent is that `missing_velocities=maxwell_boltzmann` emits the **raw** per-atom sample and does **not** remove centre-of-mass drift (D43): the correction is an MD-engine convenience that would silently change the output the user asked for. The corollary, already implied by **P1**/**P4** and now stated plainly, is that *every* augmentation Xtalate adds is reported in the Conversion Report in plain language (e.g. "Filled masses for N atoms using IUPAC standard atomic weights"). Velocity/mass fabrication is recorded with rejected alternatives in `docs/DECISIONS.md` D43–D47.

16. **Revision 1.11 — cross-format round-trip matrix suites + custom tolerance-table files (v0.2 M9).** The Part 8 §2 round-trip strategy grows from v0.1's identity round-trips to the **cross-format matrix** that catches parser/exporter *asymmetry*: a **two-hop** suite (`A → Canonical → B → Canonical′`, §2.2) parametrized over every `(source, target)` pair **enumerated from the registry**, and a **three-hop** return (`A → B → A`, §2.3) over the curated high-risk pairs, both run under the **`strict`** tolerance profile. The load-bearing rule is the milestone's cut line: the comparable subspace is **computed from the Capability Matrix at test time, never hand-listed per pair** — so a newly registered format joins the suites with zero edits (**P6**), proven by a dummy-format enumeration guard. The two-hop diff reuses the Conversion + Validation engines (the engine's `canonical′` *is* the matrix-projected subspace, and `absence_conformance` already asserts inexpressible fields absent); the three-hop endpoint diff uses a dedicated golden-anchored comparator, since it returns to format A (`docs/DECISIONS.md` D49). Separately, the review §4.4 deferral lands on schedule: **`--tolerance-profile FILE`** reads a YAML/JSON custom tolerance table (`ToleranceProfile.from_mapping`) of per-quantity `{warn, fail}` overrides on the `default` bases; the two non-configurable §4.4 rules (the fixed `k_warn`/`k_fail` multipliers and the representational-bound floor; discrete checks admit no tolerance) are enforced by rejection with actionable errors, and the profile in force is embedded verbatim in the Validation Report exactly as a named profile is (§3, §4.4). **New dependency:** PyYAML joins `[project].dependencies` — the config language for tolerance tables and the golden-corpus manifests M11 governs, chosen over a JSON-only table to avoid two config languages (`docs/DECISIONS.md` D48). No normative report/field shapes change; the round-trip suites and the tolerance-file loader are additive. *(Standing rule check: this revision touches Part 8 and Part 5 §4.4, not Parts 6/7/9, so no `IMPLEMENTATION_PLAN` propagation is required.)*
17. **Revision 1.12 — report-completeness property test (v0.2 M10).** The Part 8 §1.2 property-based test lands as `tests/property/`: the two completeness properties (Property 1 — the completeness invariant, every source-`present`/`mixed` path in `preserved` ∪ `removed` and every `supplied` entry tracing a source-`absent` path to a recorded Assumption; Property 2 — absence conformance, every `removed` path absent in the re-parsed output) are **re-derived independently in test code** (`tests/property/_properties.py`), deliberately *not* calling the runtime `_assert_completeness`/`_check_absence`, so a bug in the production guard cannot hide from its own check. Two generator stages drive them: **stage 1** parametrized golden mutations over the whole optional-field lattice, and **stage 2** a `hypothesis` strategy over randomized Canonical Objects with shrinking (`hypothesis` added as a **test-only** dev dependency; the minimal-dependency posture governs runtime deps, unchanged). Stage 2 immediately caught three real `frame_selection`-interaction defects — two `constraint_representation` silent-loss bugs and a `mixed`-cell `missing_lattice` crash — fixed with committed regression fixtures. The completeness invariant is relaxed (identically in the runtime guard and the test re-derivation) to allow a path in **both** `removed` and `supplied` when a `mixed` cell's real value is dropped by `frame_selection` and a replacement fabricated for the retained frame — both facts are in the report, so it is honest, not silent. Recorded with rejected alternatives in `docs/DECISIONS.md` D50 (the two-implementations rule + stages) and D51 (`missing_lattice` offered on a not-uniformly-present field and resolved lazily against the retained frame — fabricate / refuse / no-op). No normative report/field shapes change. *(Standing rule check: touches Part 8 §1.2, not Parts 6/7/9 — no `IMPLEMENTATION_PLAN` propagation required.)*
18. **Revision 1.13 — golden-corpus governance, contributor surface, and the v0.2 release (v0.2 M11).** The last milestone of the v0.2 break mechanizes the Part 8 §3 sourcing/licensing/versioning policy and lands the open-source contributor surface (Part 10 §4), then tags the release. **Governance as pytest** (`tests/golden/test_corpus_governance.py` over `tests/golden/_governance.py`): every `manifest.yaml` is schema-validated (required fields incl. `origin.kind` in the admissible vocabulary and a **non-empty `license`** — *no manifest, no license, no merge*, §3.2); the recorded `sha256` of each source file is re-verified so a silent fixture edit is impossible; every `expected.canonical.json` loads **through the migration chain** (`load_expected_through_migration_chain` — the identity today for the single pre-1.0 schema version, the documented seam for real migrations, §3.3) with an embedded-vs-declared `schema_version` cross-check and a one-major-version lag bound; and `tests/golden/ATTRIBUTIONS.md` is **generated from the manifests and diffed in CI**, so an attribution obligation can never silently lapse (the same regenerate-and-diff discipline as the Part 3 §3 capability-table sync test). **CI gates promoted** (Part 10 §1 deferral table, earliest v0.2): a coverage **ratchet** (`--cov-fail-under` in `pyproject.toml` addopts, set below the measured coverage with slack — a floor that rises, never lowers; `pytest-cov` added to the dev extra) and the M0 import-linter contract confirmed as a required check. **Contributor surface:** `CONTRIBUTING.md` (§4.3, incl. the two honesty clauses — golden-corpus contributions invited now, parser contributions welcome-with-churn-warning until the SDK freezes at v1.0, R12), issue templates (§4.4) and a PR template (§4.5) with the contributed-file **license-grant checkbox**. **Release:** version bumped to `0.2.0`, README scope statement updated honestly; the git tag and PyPI/GitHub publish remain the maintainer's manual step. Recorded with rejected alternatives in `docs/DECISIONS.md` D52. No normative report/field shapes change; the schema stays `0.1.0`. *(Standing rule check: touches Part 8 §3 and Part 10 §4, not Parts 6/7/9 — no `IMPLEMENTATION_PLAN` propagation required.)*
19. **Revision 1.14 — frame-chunked (streaming) processing core (v0.3 M12).** The first v0.3 milestone lands the engine change the version stands on: memory growth becomes **sub-linear in frames** through the pipeline, behind the *same* stable interfaces (Part 4 §6, P2/P6). The plugin surface (Part 3 §2) gains **additive, optional** streaming methods — `ParserPlugin.parse_stream` and `ExporterPlugin.export_stream`, each gated by `supports_streaming()` — in the same additive spirit as the existing `parse_recover`/`atom_permutation` hooks (D23/D38); a whole-file plugin that implements neither is adapted uniformly by a **named materializing fallback** (`sdk.streaming.stream_of`/`materialize`), the current behavior, no longer the only path. A `FrameStream` splits an **eager header** (object-level metadata whose size is frame-count-independent) from a **single-pass** iterator of per-frame `StreamFrame`s. Presence over a stream is computed by single-pass accumulation (`schema.PresenceAccumulator`, reproducing `field_presence()` exactly); the Conversion Engine's streaming path (`convert_stream`) drives the *same* pre-flight (`build_preflight_from_presence`) and completeness assertion the materialized path uses, so a streamed and a materialized conversion of the same input produce the **byte-identical Conversion Report** (standing rule 3). The extXYZ parser/exporter gain byte-identical streaming paths (the parser reads one frame block at a time, honoring the Part 3 §5 error contract mid-stream); validation is chunk-aware too (`validation.streaming.validate_stream` diffs expected and re-parsed-output frames pairwise over streams, producing the byte-identical Part 5 `ValidationReport` the batch engine would), so `convert_stream` is sub-linear end to end. The memory model and its measured proof are recorded in `docs/MEMORY_CEILING.md`, validated by the M12 gate test against a committed deterministic generator. Mid-stream errors honor the Part 3 §5 contract: a `ParseError` at frame k propagates out of `convert_stream`, the partial output is discarded, and no `ConversionResult` is returned — no half-written output masquerades as a completed conversion. Recorded with rejected alternatives (a lazy-`frames` `CanonicalObject`; a second parallel pipeline) in `docs/DECISIONS.md` D56, which also scopes the one deliverable deferred to M13 — the streaming `frame_selection`/`bounding_box` recovery interplay, exercised by XDATCAR→POSCAR (the seam is in place; `convert_stream` refuses such pairs to the materialized `convert` meanwhile). No normative report/field shapes change; the schema stays `0.1.0`. *(Standing rule check: additive Part 3 §2 plugin-surface extension, touches Part 4 §6 and Part 5 §2, not Parts 6/7/9 — no `IMPLEMENTATION_PLAN` propagation required.)*
20. **Revision 1.15 — XDATCAR (v0.3 M13): the streaming-first parser/exporter plus the two streaming-recovery halves M12 deferred.** The critical-path v0.3 milestone adds XDATCAR — the seventh Phase-1 format and the one whose ordinary size (10⁴ configurations) forced chunking — as a **streaming-first** parser/exporter (header-eager, configuration-lazy; both fixed-cell and per-frame-cell NpT forms; `trajectory.timestep = None`, since XDATCAR numbers configurations but declares no time axis) and lands the two streaming-recovery deliverables D56 parked for it. **(1)** The `truncate_at_last_valid_frame` recovery (the opt-in half of M12's mid-stream error contract): a recoverable torn-write `ParseError` mid-stream ends the stream at the last good frame under an explicit `truncate` choice, recording the kept prefix as a `truncate_corrupt_tail` Assumption and the dropped tail as an `XDATCAR_TRUNCATED` warning — never silent (P1), never the default (refusal stands without a preset). **(2)** Single-pass streaming `frame_selection` into a single-structure target (`ConversionEngine.convert_stream_select`, the XDATCAR→POSCAR case): `first`/`last`/`index` by bounded lookback, holding one retained frame, producing a Conversion Report and output bytes **byte-identical** to the materialized `convert` (standing rule 3) via a frame_selection accounting factored to run off a `PresenceMap` (`recovery.frame_selection_stream_records`) shared by both paths. The path narrows to what it can reproduce faithfully — `split_all` and any file whose retained frame would still need a *fabricative* recovery are handed back to the materialized `convert` — so `bounding_box`-on-the-selected-frame is the general seam D56 named but one that never fires for XDATCAR (always lattice-bearing). The memory proof is extended with an XDATCAR→extXYZ gate (`docs/MEMORY_CEILING.md`; 10⁴ configurations stream at ~0 MB over baseline vs ~200 MB materialized, byte-identical output). Recorded with rejected alternatives in `docs/DECISIONS.md` D57. No normative report/field shapes change; the schema stays `0.1.0`. *(Standing rule check: additive Part 3 §2 plugin surface + one format, touches Part 4 §3.3 and §6, not Parts 6/7/9 — no `IMPLEMENTATION_PLAN` propagation required.)*
21. **Revision 1.16 — entry-point plugin discovery proven end-to-end, the §7.2 worked-example module-name correction, and the v0.3 release (v0.3 M16).** The §7.1 entry-point discovery mechanism — described normatively since Revision 1.2 but exercised through v0.2 only by first-party in-code registration — is now **implemented and proven against a real installed distribution.** `default_registry()` loads the `xtalate.parsers`/`xtalate.exporters` entry points as an additive third pass, validating each `capabilities()` declaration and reusing the ordinary `register_*` path so the duplicate-id and declaration guards cover third-party plugins for free (M16A); an installable, **public-SDK-only** example distribution (`tests/fixtures/xtalate_toyfmt`) is then discovered, sniffed, matrix-queried, and round-tripped through the full pipeline in CI (M16B). Recorded with rejected alternatives in `docs/DECISIONS.md` D60–D61. **Spec text corrected:** the §7.2 minimal worked example's `import` lines predated the shipped `src/` package layout (Part 1 §5) and named modules that never shipped under those names — `xtalate.plugin_sdk`, `xtalate.canonical_schema`, `xtalate.capability_matrix`. They now name the real modules a plugin author imports: `ParserPlugin`/`ParseResult`/`ParseIssue` **and** the `FormatCapabilities`/`FieldCapability` capability-declaration model both come from **`xtalate.sdk`** — the declaration model is deliberately placed in `sdk`, not the `capabilities` registry package (Revision 1.2), so a plugin can declare capabilities without importing the registry; `xtalate.capabilities` is the *consuming* Capability Matrix (`Registry`/`CapabilityMatrix`) — and the canonical types come from **`xtalate.schema`**. The descriptive `packages/plugin-sdk`-style labels elsewhere in Parts 3–4 are prose component names (the `canonical-schema`/`plugin-sdk`/`capability-matrix` aliases of Part 1 §5), not import paths, and are intentionally unchanged. `CONTRIBUTING.md` gains an entry-point packaging guide pointing at the worked example, carrying the R12 honesty clause (the plugin SDK is not frozen until v1.0). **Release:** version bumped to `0.3.0`, closing the v0.3 line — the frame-chunked streaming core (M12, Rev 1.14), XDATCAR (M13, Rev 1.15), the ASE `.traj` format (M14, bringing the registered set to **six** of the seven Phase-1 formats; `docs/DECISIONS.md` D58–D59), and the benchmark corpus + PR/nightly test-matrix split (M15) — with **CIF the sole remaining Phase-1 format (v0.4).** The git tag and PyPI/GitHub publish remain the maintainer's manual step (D52 precedent). No normative report/field shapes change; the schema stays `0.1.0`. *(Standing rule check: corrects a Part 3 §7.2 illustrative code block and records an already-normative §7.1 mechanism now implemented, touches no Part 6/7/9 normative surface — no `IMPLEMENTATION_PLAN` propagation required.)*
...
The first clause is the original working mission ("the trusted translation layer between computational chemistry file formats") and it survives intact: it is short, accurate, and positions the project correctly as *infrastructure* rather than an application. The appended clause is a deliberate refinement. **Justification for the change:** "trusted" is an aspiration, not a mechanism. Anyone can claim trust; Xtalate *earns* it through a specific, verifiable behavior — total transparency about information preservation and loss. Stating that mechanism in the mission sentence means a reader who sees only this one line still understands what makes Xtalate different from every existing converter. If a single-clause version is ever needed (e.g., a GitHub repo description), the first clause stands alone.

**Project name.** The brief invites better alternatives to the working title. Candidates were considered and rejected: *FormatBridge* (drops the domain), *ChemPort* (connotes I/O, not translation), *ChemLingua*-style coinages (opaque to the target personas). "Xtalate" states the mission in two morphemes — "Chem" scopes the domain, "Bridge" states the function — and is short, pronounceable, and searchable. **The working title is retained**, subject to the routine package-namespace and trademark check that precedes the first public release (`10_Roadmap.md`).
...
| **P4** | **Recover explicitly, never guess.** | When a target format requires information the source lacks (e.g., POSCAR needs a lattice; the XYZ source has none), Xtalate never invents a value silently. It offers an explicit Recovery Workflow with scientifically reasonable options, and whatever the user chooses is recorded as an Assumption in the Conversion Report — making every conversion reproducible from its report alone. |
...
A useful litmus test used throughout the spec: *if a user diffed the source and output files by hand, would anything surprise them that Xtalate didn't already tell them about?* If yes, the design violates P1.
...
| **File Repair** | Wrap atoms into cell, center structures, deduplicate atoms, reorder species, fix malformed files | Repair *modifies* scientific data, which raises the trust bar dramatically; Xtalate must first prove it can translate without lying before it earns the right to edit. |
...
Xtalate is deliberately **not** the following, and pull requests that drift toward these should be redirected:

- **Not a DFT engine.** Xtalate never computes energies, forces, or electronic structure. It faithfully carries values computed elsewhere.
- **Not a molecular dynamics package.** It stores and translates trajectories; it never integrates equations of motion. (The one nuanced edge: Recovery options like "initialize velocities from a Maxwell–Boltzmann distribution" *generate* data — but only explicitly, at the user's request, and recorded as an Assumption per P4.) **Operative rule (Revision 1.10, `docs/DECISIONS.md` D43): Xtalate performs no unrequested transformation, even a standard MD one, when it would obfuscate or hide information.** A Recovery choice fabricates *exactly* what the user asked and nothing more: `maxwell_boltzmann` emits the raw per-atom sample and does **not** remove centre-of-mass drift (an MD-engine convenience that would silently change the requested output). Such normalization steps belong to the downstream engine, or to a separate explicit, recorded choice — never a silent add-on. And every augmentation Xtalate *does* make is reported in the Conversion Report in plain language (e.g. "Filled masses for N atoms using IUPAC standard atomic weights"), per P1/P4.
...
**How Xtalate complements existing tools.** ASE and pymatgen are outstanding *scientific computing libraries* — Swiss-army toolkits for building, manipulating, and analyzing atomistic systems, with file I/O included as one feature among dozens. Their converters are optimized for *getting on with the science*, which means they make reasonable silent decisions: dropping fields a format can't hold, filling defaults, normalizing representations. That behavior is correct for their purpose and wrong for ours. Xtalate inverts the priority: conversion transparency *is* the entire product. It uses ASE and pymatgen internally as parsing/writing workhorses where they are reliable (see `01_Architecture.md`), and wraps them in the discovery, capability, recovery, reporting, and validation machinery they were never meant to provide. A researcher might use pymatgen to *build* a structure, Xtalate to *move* it between toolchains with a full audit trail, and ASE to *run* the next calculation. The tools are collaborators, not competitors — Xtalate is the trust layer between them.
...
| **Plugin SDK** | The stable interface through which third-party developers add new parsers, exporters, and (later) analysis modules without modifying core Xtalate code. |
...
Xtalate is a pipeline with a single spine — `Native File → Parser → Canonical Object → Exporter → Target Format` — surrounded by four advisory subsystems (Information Discovery Engine, Capability Matrix, Recovery Engine, Validation Engine) that inspect or guide the spine but never bypass it. This is the physical embodiment of **P2** (no parser talks to another parser) and **P5** (know the formats before converting).
...
- **How long.** A persisted Canonical Object shares the lifecycle of the conversion's *input bytes*, not its reports: it lives for the standard retention window (`09 §5.2`; default 30 minutes — Revision 1.4) and is deleted on the same schedule as the source file, because it is derived input state, not an audit record. Reports (which *do* outlive bytes) never depend on it being present — a report is self-contained (`05 §4.5`). This is why full re-parse re-validation is time-bounded while re-thresholding is bounded only by report retention (`09 §5.2`; 30 days on the hosted instance — Revision 1.5).
...
├── pyproject.toml                # The single distribution `xtalate` (§5.2): build, deps, ruff/
...
│
├── src/xtalate/               # THE scientific core: subpackages of the one distribution (§5.2),
...
│   └── cli/                      # Appendix A: xtalate CLI. Thin presenter over the subpackages
...
**Packaging clarification: one distribution, not one distribution per `packages/*` directory.** The directories under `packages/` are Python *subpackages* of a single publishable distribution, `xtalate` — not separate PyPI projects with their own `pyproject.toml`, version number, and release cadence. This is what Part 9 §3's singular phrasing ("publish the Python packages under `packages/` to PyPI as the `xtalate` distribution") already implies but never states outright; it is made explicit here to remove the ambiguity. **Alternative rejected: eight separately-versioned distributions** (one per `packages/*` directory), on the theory that independent installability maximizes reuse. For a solo-maintained project pre-v1.0, this multiplies `pyproject.toml`s, editable-install wiring, and cross-package version pinning for zero users — nothing external depends on installing `capability-matrix` without `conversion`. The *logical* separation these directories exist for is fully delivered by the acyclic import rule (§5.1) and its CI enforcement, which works identically whether the boundary is a package or a distribution. Splitting into multiple distributions remains available later — e.g., a slim `xtalate-sdk` for third-party plugin authors once the SDK freezes (`10 §6` item 3) — as an additive packaging change behind the same import structure, not a redesign.
...
> **Document status:** Binding and normative. This document defines the Canonical Model — the single data representation into which every Parser writes and from which every Exporter reads (`01_Architecture.md §2`). It is the most load-bearing contract in Xtalate: the Information Discovery Engine, Capability Matrix, Conversion Engine, Recovery Engine, and Validation Engine all operate on the structures defined here. Field names in this document are **binding vocabulary**; later docs and code must use them verbatim.
>
...
class CanonicalObject(BaseModel):
    ...
    provenance: Provenance               # REQUIRED — always known, because Xtalate itself creates it.
...
class Cell(BaseModel):
    ...
    space_group: str | None = None       # Hermann–Mauguin symbol as *declared by the source* (e.g. CIF).
                                         #   Xtalate never computes symmetry in the MVP — carried, not derived.
```
...
**Scope note.** These fields *carry* values stated by source files; Xtalate never computes them (Non-Goals, `00_Project_Overview.md §4`). Charge/magnetization *schemes* (Mulliken vs Bader vs Hirshfeld) are recorded in `simulation.extra` when the source declares them — the value array is meaningless without its scheme, and dropping the scheme label would be silent semantic loss.
...
class ConversionRecord(BaseModel):
    ...
    tool_version: str                    # Xtalate version performing the operation.
...
class Provenance(BaseModel):
    ...
    history: list[ConversionRecord] = [] # Append-only. Every operation Xtalate performs adds one record.
```

Provenance is the only category that is **always fully populated** — not because source files provide it, but because Xtalate itself is the source of this information. It is the reproducibility backbone: a Canonical Object's history plus its Assumptions must be sufficient to explain how the object came to be (**P1**, **P4**).
...
1. **Shape-validated, semantics-free.** `custom_per_atom` arrays must have first dimension `N` (validated), `custom_per_frame` first dimension `F`. Xtalate validates shape and serializability — never meaning. It carries these arrays; it does not interpret them.
...
```json
{
  ...
  "provenance": {
    ...
    "parse_notes": [
      ...
      "scaling factor folded into lattice vectors (§4); source value: 1.0"
    ],
    "history": [
...
Points to notice: fractional `(0.5, 0.5, 0.5)` became Cartesian `(2.82, 2.82, 2.82)` at the parser boundary, with the original convention recorded in `provenance.original_coordinate_system` (§4); `pbc` is populated by *format definition* with a `parse_notes` entry, not by guess (§3.4); `trajectory: null` because a POSCAR is a single structure with no time axis (§3.2); per the carry-through routing rule (§6.1) the POSCAR **title line** (free text) is carried in `user_metadata.custom_global["poscar:comment"]` while the **scaling factor** is recorded as a `provenance` parse-note, not in `simulation.extra` — because it is *already reflected in the reconstructed lattice* (the §6.1 "only if not already reflected" clause is therefore not satisfied), so it is provenance about the encoding, not losable canonical content (`docs/DECISIONS.md` D34); neither is dropped; `space_group: null` because POSCAR declares no symmetry — even though one *could* be computed, Xtalate carries declared information only (§3.4).
...
2. **extXYZ key-value/column dependence.** extXYZ expresses velocities, forces, energy, masses, charges, time, and arbitrary metadata only when the file's `Properties=` column spec or comment-line key-value pairs declare them. Capability is `PARTIAL`: the format *can* hold these; any given file may not.
...
14. **extXYZ stress (v0.1 read behavior).** The extXYZ format *can* carry a `stress=`/`virial=` comment-line key, but the v0.1 parser does **not** populate `electronic.stress`: ASE's stress sign convention cannot be reconciled with the canonical tension-positive convention (`02 §3.7.1`) without a source-declared convention, so rather than risk a silent sign flip the parser carries stress verbatim to `user_metadata.custom_per_frame["extxyz:stress"]` with a `lossy_notes` entry. The read declaration therefore reports `electronic.stress` as `none` (§4.2), and this table cell is ○ to match. Promotion to `electronic.stress` is additive once a convention signal (or an ASE-sign fixture) exists — `docs/DECISIONS.md` D18; Preface Revision 1.2 item 14.
...
```json
{
  ...
  "fields": {
    ...
    "electronic.stress":       { "level": "none", "notes": "Carried verbatim to user_metadata.custom_per_frame[\"extxyz:stress\"] in v0.1; ASE stress sign convention unreconciled with the canonical tension-positive convention (02 §3.7.1) — D18, §3 n.14." },
...
```toml
# pyproject.toml of a third-party package, e.g. xtalate-lammps-dump
[project.entry-points."xtalate.parsers"]
lammps_dump = "xtalate_lammps_dump:LammpsDumpParser"
```

At startup, the plugin registry loads all `xtalate.parsers` (and `xtalate.exporters`) entry points, validates each `capabilities()` declaration against the canonical schema paths (§4.1), and adds the format to the Capability Matrix. From that moment the new format participates in sniffing, Discovery, conversion, validation, the `/v1/capabilities` API endpoint, and the UI's format pickers — none of which contain format-specific code (**P6**; `01_Architecture.md §6`).
...
from xtalate.sdk import (
    ParserPlugin, ParseResult, ParseIssue, FormatCapabilities, FieldCapability,
)
...
- **`/v1/validate` semantics.** Every completed conversion is validated automatically and unconditionally (`05 §3` — "there is no configuration to skip validation"); the initial `ValidationReport` arrives inside the conversion job result. The `/v1/validate` endpoint exists for **re-validation under a different `tolerance_profile`**, which is *re-thresholding of the stored measured values* (`05 §4.5`), not a re-parse — so it depends only on the stored reports and remains available even after source/output byte expiry, for as long as the report itself is retained (reports outlive bytes but are themselves deleted after the report-retention window — 30 days hosted, §5, `09 §5.2`, Revision 1.5). It fails only with `404` if the conversion is unknown or its record has passed report retention. Prior reports are retained; re-validation appends, never replaces. A full re-parse re-validation (needed only to re-verify a measurement itself, not a threshold) is the CLI's offline `validate --output --source` path (`05 §4.5`), not this endpoint.
...
class LimitsResponse(BaseModel):
    ...
    retention_minutes: int             # Stored-byte retention window (§5). Revision 1.4: renamed from retention_days — the value is now sub-day and the unit must be honest.
    awaiting_recovery_ttl_minutes: int # Expiry window for paused jobs (§3.2's expires_at). Revision 1.4: renamed from awaiting_recovery_ttl_hours, same rationale.
    report_retention_days: int | None  # Conversion records + reports deleted this many days after completion (§5; Revision 1.5). None = retained indefinitely (self-host option).
...
|---|---|---|---|
| Max upload size | 100 MB | `413 FILE_TOO_LARGE` at upload, its `details` carrying `limit_bytes` and `self_hosting_url`; streamed to object storage, never buffered whole in API memory | Hosted-instance cost ceiling and privacy posture (Revision 1.4): structures and modest trajectories convert in the browser; anything larger is deliberately redirected to the CLI and self-hosting, where no size limit exists (`09 §5.4`, `07 §2.2`) — the oversize path is a funnel to the free local path, not a dead end. Server-side processing + polling is the browser-memory strategy — the client never parses, only uploads and polls (`07_Web_UI.md §5`). |
...
| File retention (inputs & outputs) | 30 minutes; outputs additionally deleted immediately on first *completed* download (Revision 1.4) | `410 FILE_EXPIRED` / `410 OUTPUT_EXPIRED` afterward, `details.reason` distinguishing `"expired"` from `"downloaded"`; `expires_at` returned at upload; `DELETE /v1/files/{file_id}` for immediate removal | The hosted instance's privacy promise (`09_Deployment.md §5.2`): file bytes are transient by design, reports outlive bytes. Inputs, persisted Canonical Objects (`01 §4.3`), and outputs all carry the same window. An aborted download stream does not consume the output — a dropped connection must not destroy a result the user never received. |
| Report retention | 30 days after conversion completion, then the conversion record and its reports are deleted (Revision 1.5) | Daily pass of the `09 §5.2` retention sweeper over Postgres rows; surfaced as `report_retention_days` in `LimitsResponse` (§2.2), `null` = indefinite (the natural self-host setting); account deletion cascades immediately (§4) | Bounds the one artifact Revision 1.4 left unbounded. A report scoped to an anonymous session or idle key loses its audit value once its owner can no longer return, while the retained-data liability does not; 30 days comfortably covers returning to re-threshold or share a record, and the user-downloaded report JSON + offline CLI re-validation (`05 §4.5`) is the durable route after that. |
| `awaiting_recovery` job TTL | 30 minutes (then `expired` → report `status="refused"`) | Job envelope's `expires_at` (§3.2), computed as min(pause + TTL, the input's `expires_at`) so a paused job can never outlive the persisted Canonical Object it needs to resume (`01 §4.3`); surfaced as `awaiting_recovery_ttl_minutes` in `LimitsResponse` (§2.2) | A paused job holds server-side state (`01 §4.3`); an unbounded pause is a slow resource leak — and the expiry-to-refusal rule (`04 §3.2`) guarantees the timeout can never silently apply a default. Shortened from 12 hours in Revision 1.4: a pause longer than byte retention would either break the privacy promise or strand a "resumable" job whose bytes are already gone. |
...
|---|---|---|---|
| `/` | Landing | Orient and route: what Xtalate is, start a conversion, explore formats | `GET /v1/capabilities` (format count for display), `GET /v1/limits` |
...
A drop zone accepting one file, with the instance's limits shown inline before any failure can occur (max size and retention from `GET /v1/limits` — e.g. "Files up to 100 MB · deleted within 30 minutes, immediately after download · delete anytime"). During upload: a progress bar driven by upload progress events, filename, and a cancel control. On `201`, route to `/files/[file_id]`, which immediately submits `POST /v1/inspect`. Failure states render the error envelope (`06 §6`) verbatim in structure: the `code` as a badge, `message` as text, and `details` (e.g. `limit_bytes` for `FILE_TOO_LARGE`) as specifics — the UI never paraphrases error codes into vaguer language. The `FILE_TOO_LARGE` state is a redirect, not a dead end (Revision 1.4): alongside `limit_bytes`, the panel states that Xtalate has no size limit when run locally and links the CLI install and self-hosting guide (`details.self_hosting_url`, `06 §5`) — the cap is this instance's posture, not the tool's.
...
**Alternative rejected (deferred, not foreclosed): chunked/resumable uploads (e.g. tus-style).** At the 100 MB default cap (Revision 1.4), a single streamed multipart request is comfortably adequate; resumable-upload protocol support is an additive backend+client feature behind the same `POST /v1/upload` semantics if instance operators raise caps later (**P6**).
...
> **Document status:** Binding. This document specifies how Xtalate's correctness claims are enforced mechanically: the testing pyramid, the round-trip testing strategy, performance/load testing for large trajectories, golden-dataset management (sourcing, licensing, versioning), and the CI pipeline. Cross-cutting suites live in `tests/` (`tests/golden/`, `tests/roundtrip/`, `tests/performance/`); package-local unit tests live beside their packages (`01_Architecture.md §5`).
>
...
| `release.yml` | tag `v*` | Re-run full suite at the tag; publish versioned container images (`vX.Y.Z` + `latest`); publish the Python packages under `packages/` to PyPI as the `xtalate` distribution (Persona 2's `pip install xtalate`, `01 §5.1`); attach a generated changelog to the GitHub Release. |
...
2. **Large-payload movement.** 100 MB uploads (`06 §5`) streaming through short-lived function instances multiplies data transfer and cold-start cost precisely on the heaviest requests.
...
Every stored artifact — uploads, persisted Canonical Objects (`01 §4.3`), and conversion outputs — carries `expires_at` (default **30 minutes**, instance-configurable; `06 §5`; Revision 1.4). Outputs are additionally deleted **immediately after the first completed download stream** (delete-on-download); an aborted or failed stream does not consume the artifact, so a dropped connection cannot destroy a result the user never received. `DELETE /v1/files/{file_id}` performs immediate deletion for users who won't wait even that long.

**Enforcement (Revision 1.4 — a mechanism inversion, with the honesty note it requires).** Object-store bucket lifecycle rules have day granularity and cannot express a 30-minute window, so the earlier stance — "deletion is the storage platform's guarantee, not an application cron job that can silently stop running" — inverts by necessity. Primary enforcement is now application-level: synchronous deletion on download completion, plus a minute-cadence **retention sweeper** (a scheduled worker job over `expires_at` in Postgres). A **1-day bucket lifecycle rule is retained as the platform-guaranteed backstop**. Because an application sweeper *can* silently stop — exactly the failure that turns a privacy promise into a lie (**P1**) — sweep lag is exported at `/metrics` and is one of the §6.1 alerts. The honest statement of the guarantee is therefore two-tier: **30 minutes as monitored application behavior; 24 hours as the platform-enforced worst case.** The privacy copy below states the former; the backstop's existence is documented here rather than hidden.

**Reports outlive bytes deliberately:** Conversion and Validation Reports persist in PostgreSQL after file expiry (`07 §2.6` renders this as the "expired" state) — the audit trail is small, the payload is large, and the trust claim lives in the trail. What the retained record contains, precisely: field names and paths, statuses, check results and measured deviations, and any recovery-supplied parameters the user chose (e.g. a fabricated lattice's vectors) — **never atomic coordinates and never file bytes**. A user who needs zero server-side residual uses the CLI or self-hosts (§5.4); the hosted UI's privacy copy links there. Storage cost therefore scales with the *last half hour* of usage, not lifetime usage — the property that makes a zero-budget hosted instance viable at all.

**Reports expire too (Revision 1.5).** "Outlive" is not "immortal": on the hosted instance, a conversion record and its reports are deleted **30 days** after the conversion completed (`report_retention_days`, instance-configurable, `06 §5`; `null` = retain indefinitely — the natural self-host setting for a permanent lab notebook). The same retention sweeper enforces it at daily cadence; reports are Postgres rows, so no bucket rule is involved. Deleting an account deletes its records and reports immediately (cascade), not on the 30-day clock. The consequences are stated rather than implied: after report expiry, server-side re-thresholding (`06 §2`) and the history entry (`07 §2.6`) for that conversion are gone. The durable artifact is the report JSON the user downloaded — the CLI's offline re-validation (`05 §4.5`) consumes it with no server state, which is why a 30-day server window forecloses nothing. **Alternative rejected: retaining reports indefinitely (the pre-1.5 stance).** The audit value of a report scoped to an anonymous session or an idle API key decays to zero once its owner can no longer return; the retained-data liability does not. A bounded window also keeps the privacy story simple enough to state in one breath. **Alternative rejected: deleting reports with the bytes (30 minutes).** That would reduce the product's central trust artifact to a within-session receipt and destroy re-thresholding and history almost immediately — the report is the point (**P1**).

**The advertised privacy statement** — normative for the hosted instance's landing page, upload page (`07 §2.2`), and README:

> **Privacy:** uploaded files and converted outputs are automatically deleted within 30 minutes — outputs immediately after you download them. File contents are never retained. What persists is the conversion's loss-accounting report (field names, check results, and any recovery parameters you chose) — never coordinates, never file bytes — and that report is itself deleted after 30 days.

**Alternative rejected: advertising "no data is retained after conversion."** Shorter and more reassuring, but false — reports outlive bytes by design, and a recovery-supplied lattice is data. A privacy claim that overstates is precisely the silent dishonesty **P1** exists to prevent; the accurate claim is nearly as strong, and has the advantage of being true.
...
1. **Confidentiality** (files are unpublished research data): private buckets only — no public object URLs ever; downloads stream through the authenticated API (or short-lived pre-signed URLs minted per request, an equivalent control); encryption at rest via the storage platform; ownership scoping per `06 §4`; the minutes-scale retention and delete-on-download policy (§5.2) is itself the largest confidentiality control — data that no longer exists cannot leak.
...
| Template | Key fields |
|---|---|
| **Bug: incorrect conversion** | Source file (or minimal reproducer), target format, the **Conversion Report and Validation Report JSON** (the reports are designed to be the bug report, `04 §2`, `05 §3`), expected vs observed, Xtalate + schema versions. |
...
| Non-goals and how Xtalate complements ASE/pymatgen | Part 0 §4; Part 1 §4.2 (wrapping boundary); Part 3 §2 (default laundering) |
